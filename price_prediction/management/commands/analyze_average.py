...
import numpy as np
from price_prediction.models import LaborCategory, OverallCenter
import shutil
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        spread_data = OverallCenter.objects.all()
        categories = list(set([elem.labor_category for elem in spread_data]))
        overall_spread = []
        data_by_year = {}
        for category in categories:
            tmp = []
            objects = OverallCenter.objects.filter(labor_category=category).order_by("year").all()
            for obj in objects:
                if not is_nan(float(obj.center)):
                    tmp.append(round(float(obj.center), 2))
                    if not obj.year in data_by_year.keys():
                        data_by_year[int(obj.year)] = [round(float(obj.center), 2)]
                    else:
                        data_by_year[int(obj.year)].append(round(float(obj.center), 2))
        ...
        years.sort()
        print("analysis of change in average from beginning to end by labor category:")
        print("average change in average from beginning of record to end",st.median(spread_change_first_to_last))
        print("analysis of change in average from beginning to end by year:")
        print("average change in average from beginning of record to end",st.median(data_by_year[years[-1]]) - st.median(data_by_year[years[0]]))
        
        print("analysis of most recent change in average by labor category:")
        print("most recent average change in average over average",st.median(most_recent_change))
        print("analysis of most recent change in average by year:")
        print("most recent average change in average over average by year",st.median(data_by_year[years[-1]]) - st.median(data_by_year[years[-2]]))
        print("analysis of change in average year over year:")
        ...
        for index in range(len(years)):
            try:
                print(years[index],":",st.median(data_by_year[years[index]]))
            except:
                break
        import code
