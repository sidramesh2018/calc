...
import numpy as np
from price_prediction.models import LaborCategory, OverallSpread, OverallCenter
import plotly
            ...
            if elem.date.year==year:
                try:
                    if float(elem.price) > 501.00 or is_nan(float(elem.price)):
                        continue
                    prices.append(inflation.inflate(round(float(elem.price),2), dt.date(year=year,month=1,day=1), dt.date(year=base_year, month=1, day=1), 'United States'))
                except:
                    import code
                    code.interact(local=locals())
        if len(prices) > 8 and stats.normaltest(prices).pvalue > 0.05:
            spread = OverallCenter(labor_category=category, year=year, center=st.mean(prices))
            spread.save()
        elif len(prices) > 3:
            if not is_nan(st.median(prices)):
                spread = OverallCenter(labor_category=category, year=year, center=st.median(prices))
                spread.save()
            else:
                continue
        else:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        OverallCenter.objects.all().delete()
